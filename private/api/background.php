<?php
	...
	function moving(GDImage $image, int $size, int $x, int $y, string $text, string $font, int $iteration = 10, int $steps = 20, int $alpha_steps = 30, bool $inverted = false) {
		for($i = 0; $i < $iteration; $i++) {
			...
			imagettftext($image, $size, 0, $x+$offset_x, $y, imagecolorallocatealpha($image, 255, 255, 255, min(127, 20 + $i * $alpha_steps)), $_SERVER['DOCUMENT_ROOT']."/public/css/fonts/{$font}.ttf", $text);
		}
	...
	imagettftext($image, 24, 0, 970-150, 220-20, imagecolorallocatealpha($image, 60, 60, 60, 0), $_SERVER['DOCUMENT_ROOT']."/public/css/fonts/SplendidB.ttf", "ANORRL");
