...
if(isset($_POST['ANORRL$EditItem$Name']) &&
	...
	if($asset->type == AssetType::PLACE && isset($_POST['ANORRL$EditItem$Place$ServerSize'])) {
		...
		// Handle YouTube trailer link
		$youtube_link = trim($_POST['ANORRL$EditItem$Place$YouTubeLink'] ?? '');
		$youtube_id = null;

		if (!empty($youtube_link)) {
			$youtube_id = extractYouTubeID($youtube_link);
			if ($youtube_id === false) {
				$_SESSION['ANORRL$EditItem$Error'] = "Invalid YouTube link.";
				$_SESSION['ANORRL$EditItem$Success'] = false;
				die(header("Location: /edit?id=$id"));
			}
		}

		// Update place settings including the new youtube_id column
		$stmt = $con->prepare('UPDATE `asset_places` SET
		`place_copylocked` = ?,
		`place_serversize` = ?,
		`place_original` = ?,
		`place_gears_enabled` = ?,
		`place_youtube_id` = ?
		WHERE `place_id` = ?;');
		$stmt->bind_param('iiiisi', $copylocked, $server_size, $original, $gears, $youtube_id, $id);
		$stmt->execute();
	...
	$(function() {
		$('input[name="ANORRL$EditItem$Place$YouTubeLink"]').on('blur', function() {
			var input = $(this);
			...
			if (match && match[1]) {
				input.val('https://youtu.be/' + match[1]);
			}
		});
	});
	...
	<td>
	<input type="text"
	name="ANORRL$EditItem$Place$YouTubeLink"
	value="<?= htmlspecialchars($asset->youtube_id ? 'https://youtu.be/'.$asset->youtube_id : '') ?>"
